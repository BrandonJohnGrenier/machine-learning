...
import ai.brandon.excel.adapter.ExcelNodeAdapter;
import ai.brandon.excel.model.ExcelSpreadsheet;
import au.com.enterprisegenetics.rest.api.NodeRepresentation;

public class ExcelNodeAdapterTest extends AdapterTest {

	private ExcelNodeAdapter adapter;

	@Before
	public void before() {
		this.adapter = new ExcelNodeAdapter();
	}

	@Test
	public void shouldRejectASpreadsheetWithMoreThanOneIdColumn() {
		AdapterResult<NodeRepresentation> result = adapter.adapt(new ExcelSpreadsheet(path("nodes-with-multiple-id-tags.xlsx")).getWorksheets().get(0));
	...
	public void shouldNotGenerateAKeyForEachNodeWhenTheMetaModelRowDoesNotContainAnIdAnnotation() {
		AdapterResult<NodeRepresentation> result = adapter.adapt(new ExcelSpreadsheet(path("missing-id-tag.xlsx")).getWorksheets().get(0));
		assertThat(result).accepted();
		assertThat(result).hasThisManyResults(2);

		for (NodeRepresentation node : result.getData()) {
